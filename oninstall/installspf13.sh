...
clone_repo() {
    ...
    if [ ! -e "$app_dir" ]; then
        git clone --depth 1 --recursive -b "$git_branch" "$git_uri" "$app_dir"
        ret="$?"
        success "$1"
        debug
    else
...
clone_neobundle() {
    if [ ! -e "$HOME/.vim/bundle/neobundle.vim" ]; then
        git clone --depth 1 $VUNDLE_URI "$HOME/.vim/bundle/neobundle.vim"
    else
...
create_symlinks() {
    ...
    # Use homeshick to symlink our config files instead of the fork ones
...
setup_neobundle() {
    system_shell="$SHELL"
    export SHELL='/bin/sh'
    # install vimproc first so the other bundles are done in parallel
    vim \
